@extends('ecom.frontend.layouts.app')
            ...
            <div class="row cols-xs-space cols-sm-space cols-md-space">
                <div class="col-lg-3 d-none d-lg-block">
                    @if(Auth::user()->user_type == 'seller')
                        @include('ecom.frontend.inc.seller_side_nav')
                    @elseif(Auth::user()->user_type == 'customer')
                        @include('ecom.frontend.inc.customer_side_nav')
                    @endif
                </div>
                    ...
                    <div class="main-content">
                                ...
                                <div class="col-md-6 col-12">
                                    <div class="float-md-right">
                                        <ul class="breadcrumb">
                                            ...
                                            <li class="active"><a href="{{ route('trading.user.index') }}">{{__('Trading System')}}</a></li>
                                        </ul>
                                        <br>
                                    </div>
                                </div>
                        ...
                        <div class="row">
                            <div class="col-md-4 offset-md-2">
                                <div class="dashboard-widget text-center green-widget text-white mt-4 c-pointer">
                                    ...
                                    <span class="d-block title heading-3 strong-400">{{-- single_price(Auth::user()->trading_user->balance) --}}</span>
                                    <span class="d-block sub-title">{{ __('Trading Balance') }}</span>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <a href="{{ route('trading.payment_settings') }}">
                           ...
                           <div class="row">
                                @php
                                    if(Auth::user()->referral_code == null){
                                        ...
                                        Auth::user()->referral_code = substr(str_shuffle(str_repeat($pool, 5)), 0, 6);
                                        Auth::user()->save();
                                    }
                        ...
                        <div class="card no-border mt-5">
                            <div class="card-header py-3">
                                <h4 class="mb-0 h6">{{__('Trading payment history')}}</h4>
                            </div>
                            <div class="card-body">
                                <table class="table table-sm table-responsive-md mb-0">
                                    ...
                                    <tbody>
                                        @if(count($trading_payments) > 0)
                                            @foreach ($trading_payments as $key => $trading_payment)
                                                <tr>
                                                    <td>{{ $key+1 }}</td>
                                                    <td>{{ date('d-m-Y', strtotime($trading_payment->created_at)) }}</td>
                                                    <td>{{ single_price($trading_payment->amount) }}</td>
                                                    <td>{{ ucfirst(str_replace('_', ' ', $trading_payment ->payment_method)) }}</td>
                                                </tr>
                                            @endforeach
                                        @else
                                            <tr>
